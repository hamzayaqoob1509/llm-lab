...
export function ExperimentsList() {
	const { data, isLoading, error } = useSWR<{ experiments: Item[] }>(\"/api/experiments\", (url) => fetch(url).then(r=>r.json()), { refreshInterval: 4000 });
				...
				<Link key={e.id} href={`/experiments/${e.id}`} className=\"block px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-900\">
					<div className=\"flex items-center justify-between gap-3\">
						...
						<span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
							...
							e.status === \"RUNNING\" ? \"bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300\" :
