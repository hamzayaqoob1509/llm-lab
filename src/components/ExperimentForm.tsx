...
export function ExperimentForm() {
	...
	async function onSubmit(e: React.FormEvent) {
		...
		try {
			const res = await fetch(\"/api/experiments\", {
				...
				body: JSON.stringify({
					prompt, model, tempMin, tempMax, tempStep, topPMin, topPMax, topPStep,
				}),
			});
	...
	return (
		<form onSubmit={onSubmit} className=\"rounded-lg border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950\">
			...
			<textarea value={prompt} onChange={(e)=>setPrompt(e.target.value)} required rows={6} placeholder=\"Explain the trade-offs between temperature and top_p for creative writing vs factual QA...\" className=\"mt-1 w-full resize-y rounded-md border border-zinc-300 bg-white p-2 text-sm outline-none ring-0 placeholder:text-zinc-400 focus:border-zinc-400 dark:border-zinc-700 dark:bg-black dark:text-zinc-100\" />
...
function NumberField(props: { label: string; value: number; setValue: (n:number)=>void; step: number; min: number; max: number; }) {
	...
	return (
		<div>
			...
			<input type=\"number\" step={step} min={min} max={max} value={value} onChange={(e)=>setValue(parseFloat(e.target.value))} className=\"mt-1 w-full rounded-md border border-zinc-300 bg-white p-2 text-sm dark:border-zinc-700 dark:bg-black dark:text-zinc-100\" />
		</div>
	);
}
